...
function AllCategories({ allCats }: props) {
    return (
        <section className='grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 lg:max-w-[1000px] lg:mx-auto gap-5 mt-10 justify-items-center place-items-center'>
            ...
            {allCats.map(cat => (
                <Link key={cat._id.toString()} href={`/categories/${cat.shortName}`} className='h-full flex flex-col col-span-1 w-full gap-5 items-center justify-center hover:scale-105 transition-all duration-200 hover:border-main rounded-md shadow-xl shadow-black border p-5'>
